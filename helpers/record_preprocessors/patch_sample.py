...
def slice_nparray(nparray, normalized_roi, accept_ndims=[2, 3], force_even_slices=False, channel_axis=0):
    if nparray is None:
    ...
    if nparray.ndim == 2 and 2 in accept_ndims:
        ...
        if force_even_slices:
            ...
            new_dim = [dims[0] - 2 * top_margin,  dims[1] - 2 * left_margin]
            top = int(norm_roi_top * new_dim[0])
            left = int(norm_roi_left * new_dim[1])
            height = int(norm_roi_height * new_dim[0])
            width = int(norm_roi_width * new_dim[1])
        else:
        ...
        if force_even_slices:
            ...
            new_dim = [dims[0], dims[1] - 2 * top_margin,  dims[2] - 2 * left_margin]
            top = int(norm_roi_top * new_dim[1])
            left = int(norm_roi_left * new_dim[2])
            height = int(norm_roi_height * new_dim[1])
            width = int(norm_roi_width * new_dim[2])
        else:
            # Else, if this is a border patch, extend the ROI with the appropriate indivisible amount.
    ...
    def _preprocess_datapoint(self, parent_datapoint, pg_session):
        ...
        for patch_x in range(0, num_patches_x):
            for patch_y in range(0, num_patches_y):
                ...
                if new_datapoint_record['metadata'] is None: # metadata can be present and None
                    new_datapoint_record['metadata'] = {}
                ...
                for parent_prediction in pg_session.query(Prediction).filter(Prediction.datapoint == parent_datapoint.id).all():
                    ...
                    if parent_prediction.encoded_segmentation_class_mask is not None:
                        ...
                        new_prediction_record['segmentation_class_mask'] = slice_nparray(
                            segmentation_class_mask, normalized_roi, accept_ndims=[2]).tolist()
                    ...
                    if feature_vector is not None:
                        ...
                        new_prediction_record['embeddings'] = slice_nparray(
                            embeddings, normalized_roi).tolist()
                ...
                for parent_groundtruth in pg_session.query(GroundTruth).filter(GroundTruth.datapoint == parent_datapoint.id).all():
                    ...
                    if parent_groundtruth.encoded_segmentation_class_mask is not None:
                        ...
                        new_groundtruth_record['segmentation_class_mask'] = slice_nparray(
                            segmentation_class_mask, normalized_roi, accept_ndims=[2]).tolist()
            ...
            for child_prediction in pg_session.query(Prediction.model_name, Prediction.id).filter(Prediction.datapoint == child_datapoint.id).all():
                ...
                if parent_vector is not None:
                    ...
                    pg_session.query(FeatureVector).filter(FeatureVector.prediction == child_prediction.id).update({
                        FeatureVector.encoded_value: encode_np_array(
                            slice_nparray(logits, normalized_roi, force_even_slices=True))
                    })
                ...
                if parent_vector is not None:
                    ...
                    pg_session.query(FeatureVector).filter(FeatureVector.prediction == child_prediction.id).update({
                        FeatureVector.encoded_value: encode_np_array(
                            slice_nparray(embeddings, normalized_roi, force_even_slices=True))
                    })
